...
load_messages() {
  if [ "$APP_LANG" = "es" ]; then
    MSG_WELCOME="===== HERRAMIENTA DE PREPARACIÓN DE VM 1.1 ====="
    ...
    MSG_WELCOME="===== VM PREPARATION TOOL 1.1 ====="
    MSG_PRECHECK="Pre-check"
    MSG_FIX="Execute FIX"
    MSG_POSTCHECK="Post-check"
...
safe_boot_cleanup() {
  local rc=0
  local keep_kernels tmp_keep file kernel_base orphan_found=0 rescue_found=0
  local free_after
